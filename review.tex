...
%DIF ADD size_and_opacity.tex              Mon Nov 27 14:10:52 2023
...
been extensively studied and are }\DIFdelend used for a \DIFaddbegin \DIFadd{wide }\DIFaddend variety of communicative tasks\DIFdelbegin \DIFdel{. }\DIFdelend \DIFaddbegin \DIFadd{, both in
academic and non-academic contexts. This ubiquity means they have been
studied extensively. }\DIFaddend While most commonly used to represent linear
...
correlation significantly better than \DIFdelbegin \DIFdel{other }\DIFdelend \DIFaddbegin \DIFadd{competing }\DIFaddend data visualizations
...
environments, lay }\DIFdelend \DIFaddbegin \DIFadd{it is no long only professionals and
scientists that are expected to use data visualizations. Lay }\DIFaddend populations
are now expected to be able to use and accurately interpret data
visualizations on a daily basis \citep{bbc_2022}. This expectation
confers a responsibility on the part of data visualization designers to
design in such \DIFdelbegin \DIFdel{a way }\DIFdelend \DIFaddbegin \DIFadd{ways }\DIFaddend that people with \DIFdelbegin \DIFdel{little }\DIFdelend \DIFaddbegin \DIFadd{limited }\DIFaddend statistical or graph
training can expect to be able to correctly interpret data
visualizations. Doing this requires us to understand human perception\DIFdelbegin \DIFdel{and perceptual phenomena}\DIFdelend ,
apply this understanding to data visualization design, and test those
designs in rigorous empirical work. Here, we present a
...
This finding suggests that the lower \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend in the center of the
...
our smallest, lowest \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend points had very low visibility. We
...
\citep{alais_2004}, but higher levels of salience \citep{healey_2011}\DIFaddbegin \DIFadd{,
results which are supported by evidence that reaction times are slower
to smaller stimuli \mbox{%DIFAUXCMD
\citep{gramazio_2014, osaka_1976}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The predicted
...
orientations of size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay functions. \DIFaddbegin \DIFadd{Note that the use of
this terminology is unrelated to the concept of redundant encoding
\mbox{%DIFAUXCMD
\citep{nothelfer_2017}}\hskip0pt%DIFAUXCMD
. }\DIFaddend We hypothesize that: (H1) an increased reduction
in correlation estimation error will be observed when congruent \DIFdelbegin \DIFdel{standard }\DIFdelend \DIFaddbegin \DIFadd{typical
}\DIFaddend orientation decay functions are used; (H2) the use of congruent inverted
...
identified using the }\textbf{\DIFadd{buildmer}} \DIFadd{package (version 2.10.1,
\mbox{%DIFAUXCMD
\citep{voeten_buildmer}}\hskip0pt%DIFAUXCMD
) in R. This package takes a maximal random
...
however \DIFdelbegin \DIFdel{extensive
}\DIFdelend testing of these is outside the scope of the present work. We
used this equation in \DIFdelbegin \DIFdel{standard }\DIFdelend \DIFaddbegin \DIFadd{typical }\DIFaddend and inverted orientation forms applied to
point size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend in a factorial 2 x 2 design. Examples of the
...
participants. In reality minimum visible size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend would need to
...
}\textbf{\DIFadd{buildmer}} \DIFadd{and }\textbf{\DIFadd{lme4}} \DIFadd{(version 1.1-34 \mbox{%DIFAUXCMD
\citep{lme4}}\hskip0pt%DIFAUXCMD
)
packages to build a linear mixed effects model where the difference
between objective and rated }\emph{\DIFadd{r}} \DIFadd{value was predicted by the size and
opacity decay functions used. Semi-partial R\textsuperscript{2} was
calculated using the }\textbf{\DIFadd{r2glmm}} \DIFadd{package (version 0.1.2
\mbox{%DIFAUXCMD
\citep{r2glmm}}\hskip0pt%DIFAUXCMD
). The }\textbf{\DIFadd{emmeans}} \DIFadd{package (version 1.8.8
\mbox{%DIFAUXCMD
\citep{emmeans}}\hskip0pt%DIFAUXCMD
) was used to calculate pairwise comparisons between
...
}\DIFaddend function used), we do not report a \DIFaddbegin \DIFadd{traditional }\DIFaddend measure of effect size.
...
\citep{nakagawa_2013}\DIFdelbegin \DIFdel{. These values
were calculated using the }\textbf{\DIFdel{r2glmm}} %DIFAUXCMD
\DIFdel{package (version 0.1.2
\mbox{%DIFAUXCMD
\citep{r2glmm}}\hskip0pt%DIFAUXCMD
) and }\DIFdelend \DIFaddbegin \DIFadd{, which }\DIFaddend can be see in Table~\ref{tbl-sig} along
with all model statistics. \DIFdelbegin \DIFdel{The }\textbf{\DIFdel{emmeans}} %DIFAUXCMD
\DIFdel{package (version 1.8.8
\mbox{%DIFAUXCMD
\citep{emmeans}}\hskip0pt%DIFAUXCMD
) was used to calculate pairwise }\DIFdelend \DIFaddbegin \DIFadd{Pairwise }\DIFaddend comparisons between levels of the
size and \DIFdelbegin \DIFdel{contrast decay factor , and }\DIFdelend \DIFaddbegin \DIFadd{opacity decay factor }\DIFaddend can be seen in Table~\ref{tbl-contrasts}.
...
and being less vigilant as a result. Regardless of its significance, }\DIFaddend we
do not consider \DIFdelbegin \DIFdel{it }\DIFdelend \DIFaddbegin \DIFadd{this effect }\DIFaddend large enough to warrant further \DIFdelbegin \DIFdel{discussion}\DIFdelend \DIFaddbegin \DIFadd{analysis,
although future work will take into account potential effects of
experimental length}\DIFaddend .
...
underestimation of correlation in scatterplots. \DIFaddbeginFL \DIFaddFL{The difference in
areas-under-the-curve between the required and observed power is shown
on each plot.}\DIFaddendFL }
...
congruent \DIFdelbegin \DIFdel{standard }\DIFdelend \DIFaddbegin \DIFadd{typical }\DIFaddend and congruent inverted conditions respectively. These
...
\DIFaddbegin \DIFadd{Channels such as point size, colour, opacity, and shape have been used
...
separable scatterplot features \mbox{%DIFAUXCMD
...
the rapid and intuitive interpretation of correlation (and }\emph{\DIFadd{only}}
\DIFadd{this).
}
...
of results from those seen here. The present study }\DIFaddend opens the door for
this future work, as it provides the necessary additional data to
previous experiments using only size \citep{strain_2023b} or \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity
