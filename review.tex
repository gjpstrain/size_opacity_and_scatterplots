...
%DIF ADD size_and_opacity.tex              Wed Nov 29 17:14:31 2023
...
%DIF -------
  pdftitle={Novel Effects of Size and Opacity Adjustments in Scatterplots}, %DIF > 
%DIF -------
...
\DIFdelend \DIFaddbegin \title[Size, Opacity, and Scatterplots]{\DIFaddend Novel Effects of Size and
...
%% article.
\begin{abstract}
Changing the size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend of points on scatterplots can be used to
...
performance on \DIFdelbegin \DIFdel{a discrimination judgement task }\DIFdelend \DIFaddbegin \DIFadd{discrimination judgement tasks }\DIFaddend can also be modeled by
...
the regression line, and can be thought of as \DIFdelbegin \DIFdel{representing }\DIFdelend \DIFaddbegin \DIFadd{closely approximating }\DIFaddend the
...
aspect of \DIFdelbegin \DIFdel{scatterplot shape }\DIFdelend \DIFaddbegin \DIFadd{the shape of the point cloud }\DIFaddend to judge correlation, or some
...
Previous work has found that lowering the \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend of \emph{all}
scatterplot points relative to the background can increase the level of
underestimation error relative to full \DIFdelbegin \DIFdel{contrast}\DIFdelend \DIFaddbegin \DIFadd{opacity}\DIFaddend , and that lowering point
\DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend \emph{as a function of distance from the regression line} is
...
effect. Lower stimulus contrast\DIFdelbegin \DIFdel{is }\DIFdelend \DIFaddbegin \DIFadd{, which for isolated stimuli is
functionally identical to lower opacity, is }\DIFaddend associated with lower
...
and 7. These points were of the same size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend as the smallest
and lowest \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend points used in the experimental items. Participants
were asked to enter in a textbox how many points were present.
Participants scored an average of 74.89\% (\(SD\) = 32.25\%). Despite
our use of the \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend floor detailed in
Section~\ref{sec-transparency-and-contrast}, it is clear that some of
our small, low \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend points were not reliably visible, most likely due
...
participants. In reality minimum visible \DIFdelbegin \DIFdel{size and contrast }\DIFdelend \DIFaddbegin \DIFadd{point size and opacity }\DIFaddend would
need to be calibrated on a per-monitor basis. We also include
performance on the point visibility test as a fixed effect in
...
\DIFaddbegin \DIFadd{on a hypothetical 35.54 x 20.00cm monitor}\DIFaddend . We include analyses with dot
...
to examine responses purely as a function of }\emph{\DIFadd{when}} \DIFadd{they were made.
We find a significant effect of trial number on participant's errors
(\(\chi^2\)(1) = 29.31}\DIFaddend , \DIFaddbegin \emph{\DIFadd{p}} \DIFadd{\textless{} .001).
...
}\DIFaddend demonstrated in previous work \citep{strain_2023, strain_2023b},
although this may be a facet of the non-additive interaction between
size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend manipulations we found. Despite the lack of support for
this hypothesis, we did find that the size decay channel explained more
variance (.104) in our model than \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay (.087) was able to.
...
use of these visual \DIFdelbegin \DIFdel{factors }\DIFdelend \DIFaddbegin \DIFadd{features }\DIFaddend to be able to change people's perceptions
...
function \DIFdelbegin \DIFdel{did exhibit }\DIFdelend \DIFaddbegin \DIFadd{exhibited }\DIFaddend lower mean error than the one using inverted
...
More recent work using size or \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay functions similar to the
...
decay function, whether using \DIFdelbegin \DIFdel{standard}\DIFdelend \DIFaddbegin \DIFadd{typical}\DIFaddend /inverted orientation non-linear
functions or a linear decay function, results in \textbf{no} change in
\emph{r} estimation precision \citep{strain_2023b}. When \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend is used
...
\DIFdelbegin \DIFdel{standard orientation contrast }\DIFdelend \DIFaddbegin \DIFadd{typical orientation opacity }\DIFaddend decay function was used. We suggest this is
part of the moderating effect of the point \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay function on the
...
\DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay function normalizes this to the point where precision is
restored.
...
previous studies that \DIFaddbegin \DIFadd{the same }\DIFaddend used decay functions applied solely to
size or \DIFdelbegin \DIFdel{contrast}\DIFdelend \DIFaddbegin \DIFadd{opacity}\DIFaddend , and with no manipulations present. \DIFdelbegin \DIFdel{This final }\DIFdelend \DIFaddbegin \DIFadd{The ``no
manipulations present'' }\DIFaddend plot is averaged over conditions from \DIFdelbegin \DIFdel{previous work}\DIFdelend \DIFaddbegin \DIFadd{our
previous works (references removed for anon)}\DIFaddend .
...
their \DIFdelbegin \DIFdel{methodological similarities render comparison }\DIFdelend \DIFaddbegin \DIFadd{use of identical decay equations (albeit in isolation) and
experimental paradigms render comparisons }\DIFaddend appropriate, but we
acknowledge the potential for overstated conclusions. \DIFaddbegin \DIFadd{(references
removed for anon).
}\DIFaddend 
...
overestimation. \DIFaddbegin \DIFadd{Figure~\ref{fig-power-plot} also includes the integral
of each power curve over }\emph{\DIFadd{r}}\DIFadd{, a measure of the total power of each
curve. We also display the difference between this integral and the
integral of each required power curve over }\emph{\DIFadd{r}}\DIFadd{, which shows to what
extent the power we observed differs from what would be required.
}\DIFaddend 
...
results that for many participants, the smallest and lowest \DIFdelbegin \DIFdel{contrast
}\DIFdelend \DIFaddbegin \DIFadd{opacity
}\DIFaddend points we used were simply not visible, although it would seem that this
...
were removing data, which \DIFdelbegin \DIFdel{goes against }\DIFdelend \DIFaddbegin \DIFadd{violates }\DIFaddend our intended aims. Solving this would
require a by-participant calibration of point size and point \DIFdelbegin \DIFdel{contrast}\DIFdelend \DIFaddbegin \DIFadd{opacity}\DIFaddend ,
...
observed effect in the \DIFdelbegin \DIFdel{standard }\DIFdelend \DIFaddbegin \DIFadd{typical }\DIFaddend orientation congruent condition were due
to size or \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay. We can conclude that these effects are not
linearly additive, but must suggest further work to define precisely
each of their contributions. \DIFaddbegin \DIFadd{While our provision of dot pitch
measurements is a step in the right direction, the present methodology
leaves us unable to comment on the variability in participants' viewing
...
\DIFaddbegin \DIFadd{Due to our extensive previous testing of no-adjustment, size-only, and
opacity-only manipulation scatterplots (references removed for anon), we
choose not to include these as conditions in the present work. We did
not consider the increased costs and experimental length worth the
inclusion of three extra conditions, although we acknowledge that these
would assist us in making claims about additivity and the relative
contributions of point size and opacity decay.
}
...
interpretations may be introduced. As our contribution in this paper is
...
problem}\DIFdelend \DIFaddbegin \DIFadd{Due to evidence that
...
, the techniques we have implemented in the current
paper may be used in future (in a symmetrical manner) to address the
overestimation bias}\DIFaddend . We found evidence that the influence of size and
\DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay functions changes according to the direction they are
...
positively correlated scatterplots. For size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay in the
...
increase or decrease the severity of the \DIFdelbegin \DIFdel{effect }\DIFdelend \DIFaddbegin \DIFadd{decay function }\DIFaddend in question.
...
size or \DIFdelbegin \DIFdel{contrast. The present work
}\DIFdelend \DIFaddbegin \DIFadd{opacity. Future experimental work may use the major axis through
...
\mbox{%DIFAUXCMD
\citep{collyer_1990} }\hskip0pt%DIFAUXCMD
suggests that this may produce a different pattern
of results from those seen here. If changes in dot density are driving
changes in correlation estimates, the congruent conditions here are an
example of redundant encoding. Future work may explore using different
channels to redundantly encode dot density, such as marker
shape/orientation or colour. The present study }\DIFaddend opens the door for this
