...
%DIF ADD size_and_opacity.tex              Mon Dec 11 14:05:52 2023
...
performance \DIFdelbegin \DIFdel{on a discrimination judgement task }\DIFdelend can also be modeled by deep neural networks
...
and is \DIFdelbegin \DIFdel{demonstrated using an equation relating objective to subjective
correlation \mbox{%DIFAUXCMD
\citep{rensink_2017} }\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{illustrated }\DIFaddend in Figure~\ref{fig-underestimation-curve}. More
...
such that \DIFdelbegin \DIFdel{they }\DIFdelend \DIFaddbegin \DIFadd{their correlations }\DIFaddend are discriminable 75\% of the time
\citep{rensink_2010}. This work has \DIFdelbegin \DIFdel{also
}\DIFdelend been extended to incorporate
Bayesian data analysis \citep{kay_2015}, which \DIFdelbegin \DIFdel{is in agreement with other literature identifying }\DIFdelend \DIFaddbegin \DIFadd{also identifies
}\DIFaddend scatterplots as particularly suited for the communication of correlation
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{li_2010}}\hskip0pt%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{li_2010, harrison_2014}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The current experiment adapts techniques
...
to collect responses\DIFdelbegin \DIFdel{. This paradigm }\DIFdelend \DIFaddbegin \DIFadd{; this }\DIFaddend allows for a large number of judgements to be
...
scatterplot \DIFdelbegin \DIFdel{biased viewers' perceptions down}\DIFdelend \DIFaddbegin \DIFadd{further biased participants towards underestimation}\DIFaddend . When
point \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend or size are reduced as a function of distance from the
...
\DIFdelbegin \DIFdel{This supports
}\DIFdelend \DIFaddbegin \DIFadd{These findings support }\DIFaddend a low-level data salience account. Our aims are
...
opacity in a }\emph{\DIFadd{uniform}} \DIFadd{manner, the sheer number of points used in
that study results in scatterplots that appear similar to those that
reduce opacity as a function of residual error \mbox{%DIFAUXCMD
...
reported in previous \DIFdelbegin \DIFdel{work
}\DIFdelend \DIFaddbegin \DIFadd{studies
}\DIFaddend \citep{rensink_2017, strain_2023, strain_2023b}. We acknowledge that
...
\DIFaddbeginFL \DIFaddFL{with an }\textit{\DIFaddFL{r}} \DIFaddFL{value of 0.6}\DIFaddendFL .\DIFdelbeginFL \DIFdelFL{Congruent conditions are at the top, while incongruent conditions are
below.}\DIFdelendFL }
...
Due both to our use of a linear mixed \DIFaddbegin \DIFadd{effects }\DIFaddend model with an interaction
term, and our lack of a comparative baseline condition (i.e no size or
\DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity decay }\DIFaddend function used), we do not report a \DIFaddbegin \DIFadd{traditional }\DIFaddend measure of
...
in our model than \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay (.087)\DIFdelbegin \DIFdel{was able
to}\DIFdelend .
...
not additive}\DIFaddend . Doing this would allow us to tune each function according
to both objective \emph{r} value and the tuning of the other function to
\DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{facilitate more }\DIFaddend accurate correlation estimates. We can derive new curves
...
by comparing them to estimates without any manipulation present \DIFaddbegin \DIFadd{across
the range of }\emph{\DIFadd{r}} \DIFadd{values used}\DIFaddend . We term this `power'. The dotted line
on each plot shows the power \DIFdelbegin \DIFdel{we would need }\DIFdelend \DIFaddbegin \DIFadd{that would be needed }\DIFaddend to correct for the
standard underestimation curve (see
Figure~\ref{fig-underestimation-curve}). As we can see, size alone
provides the closest to the requirement, and combining size and \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity
}\DIFaddend decay functions results in gross overestimation.
\DIFaddbegin \DIFadd{Figure~\ref{fig-power-plot} includes the integral of each power curve
over }\emph{\DIFadd{r}}\DIFadd{, a measure of the total power of each curve. We also
display the difference between this integral and the integral of each
required-power curve over }\emph{\DIFadd{r}}\DIFadd{, which shows to what extent the power
...
in scatterplots \mbox{%DIFAUXCMD
\citep{hong_2021, smart_2019}}\hskip0pt%DIFAUXCMD
. While we focus purely on
...
scatterplot features \mbox{%DIFAUXCMD
\citep{smart_2019}}\hskip0pt%DIFAUXCMD
, if viewers assume that
...
rapid and intuitive interpretation of correlation (and }\emph{\DIFadd{only}}
\DIFadd{this).
}
...
used the same value of \emph{b} (0.25) as \DIFdelbegin \DIFdel{in }\DIFdelend previous work
