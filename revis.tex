...
%DIF ADD size_and_opacity.tex              Thu Nov 16 10:16:25 2023
...
\citep{kay_2015}, which \DIFdelbegin \DIFdel{is in agreement with other literature identifying }\DIFdelend \DIFaddbegin \DIFadd{also identifies }\DIFaddend scatterplots as particularly
...
. Given that this technique has been shown to produce
...
employed in Micallef et al. \mbox{%DIFAUXCMD
\citep{micallef_2017}
}\hskip0pt%DIFAUXCMD
}
...
\citep{gramazio_2014}}\hskip0pt%DIFAUXCMD
}\DIFaddend . The predicted effects of these
drivers on correlation perception work in the opposite direction to one
another, which has allowed researchers to provide evidence for the
mechanistic dominance of salience when point size is used. When an
inverted size decay function is used such that smaller points are
located nearer the regression line, correlation estimation is
...
point size and point \DIFdelbegin \DIFdel{contrast. }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend In our study, we combine previously
independently tested point size and point \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay functions in
both \DIFdelbegin \DIFdel{standard orientation (contrast}\DIFdelend \DIFaddbegin \DIFadd{typical orientation (opacity}\DIFaddend /size is reduced with residual
magnitude) and inverted orientation (\DIFdelbegin \DIFdel{contrast}\DIFdelend \DIFaddbegin \DIFadd{opacity}\DIFaddend /size is increased with
...
studies }{[}\DIFadd{\mbox{%DIFAUXCMD
\citet{strain_2023}}\hskip0pt%DIFAUXCMD
; strain\_2023b}{]}\DIFadd{. Each scatterplot was
created and presented using }\DIFaddend a 1:1 aspect ratio, \DIFdelbegin \DIFdel{was generated as a
}\DIFdelend \DIFaddbegin \DIFadd{in line with findings
that this facilitates optimal performance on correlation estimation
tasks \mbox{%DIFAUXCMD
\citep{micallef_2017}}\hskip0pt%DIFAUXCMD
. Scatterplots were generated as }\DIFaddend 1000 x 1000
pixel .png \DIFdelbegin \DIFdel{image, and was }\DIFdelend \DIFaddbegin \DIFadd{images, and were }\DIFaddend scaled up or down according to a
participant's monitor such that they always occupied the same proportion
of the screen. We used equation 1 to map residuals to size and \DIFdelbegin \DIFdel{contrast
values. }\DIFdelend \DIFaddbegin \DIFadd{opacity
values:
}
...
0.34mm (\(SD\) = 0.05), corresponding to a physical \DIFdelbegin \DIFdel{onscreen }\DIFdelend \DIFaddbegin \DIFadd{on-screen }\DIFaddend size of
4.39mm on a 1920 x 1080 pixel monitor for the smallest points displayed.
...
use a slider to estimate correlation to 2 decimal places. \DIFdelbegin \DIFdel{Visual masks
preceded each scatterplot }\DIFdelend \DIFaddbegin \DIFadd{Participants
...
were displayed for 1 second in between each scatterplot presentation}\DIFaddend .
...
}\DIFaddend decay provides evidence that these functions interact in non-additive
...
\item
  Outside of this range, \DIFdelbegin \DIFdel{the contrast decay functionshould be used in
...
Using \DIFdelbegin \DIFdel{contrast decay alone does not significantly change the shape of
the }\DIFdelend \DIFaddbegin \DIFadd{opacity decay alone significantly changes the amplitude of
}\DIFaddend estimation error curve, \DIFdelbegin \DIFdel{whereas using size decaydoes}\DIFdelend \DIFaddbegin \DIFadd{while leaving its shape intact (compare opacity
...
\citep{strain_2023b}. It would appear that the addition of the \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity
}\DIFaddend curve moderates the effect of the size curve as a function of the
...
Using a \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay function in isolation has a small effect on
...
(0.3 to 0.75), and to use the \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay function in isolation
...
precision with \emph{r} that the \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend decay function confers. It is
...
\DIFdelbegin \DIFdel{power
}\DIFdelend \DIFaddbegin \DIFadd{contributions }\DIFaddend of each decay function \DIFdelbegin \DIFdel{and their power together}\DIFdelend \DIFaddbegin \DIFadd{when they are used together, as
their combination is not linearly additive}\DIFaddend . Doing this would allow us to
tune each function according to both objective \emph{r} value and the
tuning of the other function to \DIFdelbegin \DIFdel{produce }\DIFdelend \DIFaddbegin \DIFadd{facilitate }\DIFaddend accurate correlation
...
\citet{hong_2021}}\hskip0pt%DIFAUXCMD
; smart\_2019}{]}\DIFadd{. While we focus purely on
...
}\DIFdelend \DIFaddbegin \DIFadd{opacity The present study }\DIFaddend opens the door for this future work,
as it provides the necessary additional data to previous experiments
using only size \citep{strain_2023b} or \DIFdelbegin \DIFdel{contrast }\DIFdelend \DIFaddbegin \DIFadd{opacity }\DIFaddend \citep{strain_2023}
decay functions. Further testing of these manipulations in isolation and
