...
adjust a plot until its correlation is halfway between two reference
...
contrast decreased the closer it was to the regression line, resulted in
significantly lower and less accurate correlation estimates compared to
data-identical plots with the same overall shape. This findings implies
that the center of the scatterplot not being \textbf{filled} biased
viewers' perceptions down. When point contrast or size are reduced as a
function of distance from the regression line
...
salience when point size is used. As with contrast, when an inverted
size decay function is used such that smaller points are located nearer
the regression line, correlation estimation is significantly more
accurate than when all points are the same size \citep{strain_2023b}. In
this case it was suggested that the higher spatial uncertainty brought
on by larger points caused a perceptual downweighting during correlation
estimation, which is in line with work suggesting our perceptual systems
make robust use of visuo-spatial information
...
different viewing contexts and the wide range of statistical and graph
experience present in lay populations. In addition, the ease and
...
0.2 to ensure that the minimum point size in the present study is both
visible and consistent with that of previous work
...
point visibility testing to test this. Participants viewed six
...
size and contrast would need to be calibrated on a per-monitor basis.
\textbf{?@fig-VT-hist} shows distributions of participants' performances
on the visibility tests. We also include performance on the point
visibility test as a fixed effect in Section~\ref{sec-add-analyses}.
...
effects model where the difference between objective and rated \emph{r}
value was predicted by the size and contrast decay conditions used. A
...
underestimation problemm, it does demonstrate that with regards to using
...
exhibit the conventional increase in precision with \emph{r}.
...
estimation curve is altered quite radically, and we fail to observe an
increase in precision with the objective \emph{r} value. We observe an
over-correction at lower values of \emph{r}, leading to an
...
\textless{} 0.8. Used together, we can exploit the power of the size
...
estimates. We can also derive new curves that describe the effect that
each manipulation and the combination of manipulations has on people's
...
contrast decay have combined to produce an estimation curve that has
...
poor (see \textbf{?@fig-VT-hist}). It is clear from these results that
