...
systematically alter viewers' perceptions of correlation. Point size and
contrast adjustments operate on correlation perception through different
perceptual mechanisms, and combining them produces effects that are not
additive in nature. We present a fully reproducible study in which we
combine previously established techniques for influencing correlation
perception to show that there are few limits to the extent to which we
can use visual features to change viewers' perceptions of data
visualizations. We follow this with an extended discussion into how our
findings corroborate previous work on correlation perception in
scatterplots and can be used to correct for a historic perceptual
bias.    
\end{abstract}
...
\caption{\label{fig-emm-plot}Estimates marginal means for each
...
for each combination of factors. The interaction is driven by the
non-additive nature of combining point size and contrast manipulations;
the effects of either on performance on the correlation estimation task
depend on the presence of the other. Table~\ref{tbl-contrasts} shows
