...
are often based on what is being presented (e.g., gratings) or are
modeled to take into account aspects of human vision (e.g., visibility
limits) \citep{zuffi_2007}. Our crowdsourced methodology gives us no
control over the exact luminances of our stimuli, only over the
\emph{relative} differences in luminance between scatterplot points and
backgrounds. For this reason we do not report absolute luminance nor
...
to smaller stimuli \citep{gramazio_2014}. The predicted effects of these
...
both typical orientation (contrast/size is reduced with residual
...
orientations of size and contrast decay functions. Note that the use of
...
studies {[}\citet{strain_2023}; strain\_2023b{]}. Each scatterplot was
created and presented using a 1:1 aspect ratio, in line with findings
that this facilitates optimal performance on correlation estimation
tasks \citep{micallef_2017}. Scatterplots were generated as 1000 x 1000
pixel .png images, and were scaled up or down according to a
...
applied to point size and contrast in a factorial 2 x 2 design. Examples
...
0.34mm (\(SD\) = 0.05), corresponding to a physical on-screen size of
...
were displayed for 1 second in between each scatterplot presentation.
...
contrast decay functions used. Semi-partial R\textsuperscript{2} was
...
levels of the size and contrast decay factor.

Our first two hypotheses were fully supported in this experiment. The
combination of typical orientation size and contrast decay functions
...
typical orientation size/inverted orientation contrast decay plots and
inverted orientation size/typical orientation contrast decay plots (Z =
...
and our lack of a comparative baseline condition (i.e no size or
contrast function used), we do not report a traditional measure of
...
comparisons between levels of the size and contrast decay factor can be
...
\item
  ...
  may wish to implement the contrast decay function; while its effect on
...
how, on average, the combination of typical orientation size and
...
typical orientation contrast decay function was used. We suggest this is
part of the moderating effect of the point contrast decay function on
...
Using contrast decay alone significantly changes the amplitude of
estimation error curve, while leaving its shape intact (compare contrast
...
manipulated and no manipulation present plots in
Figure~\ref{fig-est-multi-exp}). When size and contrast decay functions
...
\citep{strain_2023b}. It would appear that the addition of the contrast
...
require a by-participant calibration of point size and point contrast,
...
to size or contrast decay. We can conclude that these effects are not
linearly additive, but must suggest further work to define precisely
each of their contributions. Lastly, while our provision of dot pitch
...
symmetrically to that of positively correlation scatterplots
...
contrast decay functions changes according to the direction they are
...
positively correlated scatterplots. For size and contrast decay in the
...
size or contrast. The present study opens the door for this future work,
as it provides the necessary additional data to previous experiments
using only size \citep{strain_2023b} or contrast \citep{strain_2023}
