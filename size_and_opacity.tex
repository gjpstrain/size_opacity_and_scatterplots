...
viewers of scatterplots. Evidence points to the mechanisms behind these
effects being similar, so one may expect their combination to be
additive regarding their effects on correlation estimation. We present a
...
performance tasks can also be modeled by deep neural networks
...
allows for a large number of judgements to be collected, and is simple
...
significantly higher and are significantly more accurate. These findings
support a low-level data salience account. Our aims are to test the
...
decay function used), we do not report a traditional measure of effect
...
further work would need to be done to do this in
...
Figure~\ref{fig-diff-error-bars-plot}), however in each case the opacity
...
that used similar decay functions and the same experimental paradigms
...
been smoothed. Overestimation occurs above the dashed line. The curve
for the plot without manipulation was averaged from previous work.}
...
overestimation. Figure~\ref{fig-power-plot} includes the integral of
each power curve over \emph{r}, a measure of the total power of each
curve. We also display the difference between this integral and the
integral of each required power curve over \emph{r}, which shows to what
...
implement this in the future, although it would require the use of a
platform such that experimental stimuli could be regenerated per a
...
features \citep{smart_2019}, if viewers assume that variations in point
...
paper may be used (in a symmetrical manner) to address the
...
increase or decrease the severity of the decay function in question.
...
channels to redundantly encode dot density, such as marker shape,
orientation or colour. The present study opens the door for this future
work, as it provides the necessary additional data to previous
experiments using only size \citep{strain_2023b} or opacity
