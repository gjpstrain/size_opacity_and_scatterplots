...
participants. In reality minimum visible point size and opacity would
...
function exhibited lower mean error than the one using inverted
...
previous studies that the same used decay functions applied solely to
size or opacity, and with no manipulations present. The ``no
manipulations present'' plot is averaged over conditions from our
previous works (references removed for anon).
...
their use of identical decay equations (albeit in isolation) and
experimental paradigms render comparisons appropriate, but we
acknowledge the potential for overstated conclusions. (references
removed for anon).
...
overestimation. Figure~\ref{fig-power-plot} also includes the integral
of each power curve over \emph{r}, a measure of the total power of each
...
Due to our extensive previous testing of no-adjustment, size-only, and
...
shape/orientation or colour. The present study opens the door for this
